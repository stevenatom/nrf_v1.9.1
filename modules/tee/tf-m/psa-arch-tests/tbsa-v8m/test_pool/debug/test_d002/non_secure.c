...
TBSA_TEST_PUBLISH(CREATE_TEST_ID(TBSA_DEBUG_BASE, 2),
                  CREATE_TEST_TITLE("There must be a DPM to permit access to non-Trusted assets"),
                  CREATE_REF_TAG("R040_TBSA_DEBUG"),
                  entry_hook,
                  test_payload,
                  exit_hook);
dpm_hdr_t   *dpm_hdr;
...
tbsa_status_t test_env_reset(void)
{
    ...
    /* Configure the DPM to allow access to all assets.*/
    for (dpm_instance = 0; dpm_instance < dpm_hdr->num; dpm_instance++) {
        status = g_val->target_get_config(TARGET_CONFIG_CREATE_ID(GROUP_DPM, DPM_DPM, dpm_instance),
                                        (uint8_t **)&dpm_desc, (uint32_t *)sizeof(dpm_desc_t));
        if (g_val->err_check_set(TEST_CHECKPOINT_D, status)) {
            return status;
        }

        /* Configure the DPM to allow access to non-secure state only.*/
        status = g_val->dpm_set_access_ns_only(dpm_desc->index, TRUE);
...
void test_payload(tbsa_val_api_t *val)
{
    ...
    for (dpm_instance = 0; dpm_instance < dpm_hdr->num; dpm_instance++) {
        ...
        /* Configure the DPM to allow access to non-secure state only.*/
        status = val->dpm_set_access_ns_only(dpm_desc->index, TRUE);
