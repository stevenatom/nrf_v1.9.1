/*
 *  Simple DTLS server program that does the same thing as echo-server but
...
#define SRV_KEY_FILE "echo-apps-key.pem"
	...
	ret = mbedtls_x509_crt_parse_file(&srvcert, srv_cert_file);
	if (ret != 0) {
	...
	ret =  mbedtls_pk_parse_keyfile(&pkey, srv_key_file, NULL);
	if (ret != 0) {
	...
	if ((ret = mbedtls_net_bind(&listen_fd, NULL, listen_port,
