...
/* Next entry is 1232 bytes long which is the maximum length for IPv6 user
 * payload the Zephyr IP stack supports (1280 - IPv6 (40) - UDP (8))
 */
static unsigned char lorem_ipsum[1232] = \
...
/* 1232 bytes of binary data */
static unsigned char array_1232[1232] = {
...
int main(int argc, char**argv)
{
	...
	void *address = NULL;
	bool forever = true, help = false, do_reverse = false;
	int data_size = 0;
	unsigned pkt_seq = 0;

	opterr = 0;

	while ((c = getopt(argc, argv, "Fi:p:hs:")) != -1) {
		switch (c) {
		case 'F':
			flood = true;
			break;
	...
	if (!target || help) {
		printf("usage: %s [-i iface] [-F] <IPv{6|4} address of the "
		       "throughput-server>\n", argv[0]);
		...
		printf("-s Max data size to send.\n");
		exit(-EINVAL);
	}
...
again:
	do {
		...
		while (data[i].buf) {
			...
			hdr = (struct header *)data[i].buf;
			hdr->type = TYPE_SEQ_NUM;
			hdr->len = sizeof(seq);
			memcpy(hdr->value, &seq, sizeof(seq));

			ret = sendto(fd, data[i].buf, len, 0,
				     addr_send, addr_len);
			if (ret < 0) {
				perror("send");
				goto out;
			...
			/* Print some feedback to the user every X packets */
			if (!(pkt_seq % 1000)) {
				printf(".");
				fflush(stdout);
			}
			...
			if (ret < 0) {
				...
				if (!flood && data[i].expecting_reply) {
					fprintf(stderr,
