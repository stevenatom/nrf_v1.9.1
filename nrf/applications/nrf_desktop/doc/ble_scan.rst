...
#. Enable the following configuration options:
   ...
   * :kconfig:`CONFIG_BT_SCAN`
   * :kconfig:`CONFIG_BT_SCAN_FILTER_ENABLE`
...
#. If you want to limit the number of attempts to connect to a device, you can enable the connection attempt filter with the :kconfig:`CONFIG_BT_SCAN_CONN_ATTEMPTS_FILTER` Kconfig option.
   ...
   * :kconfig:`CONFIG_BT_SCAN_CONN_ATTEMPTS_FILTER_LEN` - This option defines the maximum number of filtered devices.
   * :kconfig:`CONFIG_BT_SCAN_CONN_ATTEMPTS_COUNT` - This option defines the connection attempt count for a given peripheral.
...
If :kconfig:`CONFIG_DESKTOP_BLE_NEW_PEER_SCAN_REQUEST` is enabled, you can also set the :kconfig:`CONFIG_DESKTOP_BLE_NEW_PEER_SCAN_ON_BOOT` option to make the central scan for new peers after every boot.
...
* If a peripheral is connected, the scanning is triggered periodically.
  If none of the connected peripherals is in use for at least :kconfig:`CONFIG_DESKTOP_BLE_SCAN_START_TIMEOUT_S`, the scanning is started.
...
The number of Bluetooth connections is defined as the :kconfig:`CONFIG_BT_MAX_CONN` Kconfig option.
...
* The maximum scan duration specified by :kconfig:`CONFIG_DESKTOP_BLE_SCAN_DURATION_S` times out.
