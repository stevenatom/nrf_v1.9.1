...
#define GET_RESPONSE_CALLBACKS(name)                                                                                   \
	...
	CHIP_ERROR err =                                                                                               \
		gCallbacks.GetResponseCallback(sourceId, sequenceNumber, &onSuccessCallback, &onFailureCallback);      \
                                                                                                                       \
...
#define GET_CLUSTER_RESPONSE_CALLBACKS(name)                                                                           \
	...
	/* #6559: Currently, we only have one commands for the IMInvokeCommands and to a device, so the seqNum is      \
	 * always set to 0. */                                                                                         \
...
bool emberAfOtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback(EndpointId endpoint,
									app::CommandSender *commandObj, uint8_t action,
									uint32_t delayedActionTime)
{
	...
	Callback::Callback<OtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback> *cb =
		Callback::Callback<OtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback>::FromCancelable(
			onSuccessCallback);
...
bool emberAfOtaSoftwareUpdateProviderClusterQueryImageResponseCallback(
	EndpointId endpoint, app::CommandSender *commandObj, uint8_t status, uint32_t delayedActionTime,
	...
	chip::ByteSpan updateToken, bool userConsentNeeded, chip::ByteSpan metadataForRequestor)
{
	...
	Callback::Callback<OtaSoftwareUpdateProviderClusterQueryImageResponseCallback> *cb =
		Callback::Callback<OtaSoftwareUpdateProviderClusterQueryImageResponseCallback>::FromCancelable(
			onSuccessCallback);
	cb->mCall(cb->mContext, status, delayedActionTime, imageURI, softwareVersion, softwareVersionString,
