...
namespace Controller
{
									...
									chip::ByteSpan updateToken, uint32_t newVersion)
	{
		...
		CommandSenderHandle sender(Platform::New<app::CommandSender>(mDevice->GetInteractionModelDelegate(),
									     mDevice->GetExchangeManager()));
		...
		// #6308: This is a temporary solution before we fully support IM on application side and should be
		// replaced by IMDelegate.
		...
		// We have successfully sent the command, and the callback handler will be responsible to free the
		// object, release the object now.
		sender.release();
	exit:
		return err;
	}
