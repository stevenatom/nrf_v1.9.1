...
static void bt_conn_foreach_rpc_handler(CborValue *value, void *handler_data)
{
	int type;
	bt_conn_foreach_cb func;
	void *data;

	type = ser_decode_int(value);
	func = (bt_conn_foreach_cb)ser_decode_callback(value, bt_conn_foreach_cb_encoder);
...
static void bt_conn_lookup_addr_le_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
...

	{
		...
		bt_rpc_encode_bt_conn(&ctx.encoder, result);
...
static void bt_conn_get_dst_out_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
	bool result;
	const struct bt_conn *conn;
...

	{
		...
		ser_encode_buffer(&ctx.encoder, dst, sizeof(bt_addr_le_t));
...
static void bt_conn_get_info_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
	int result;
	const struct bt_conn *conn;
...

	{
		...
		bt_conn_info_enc(&ctx.encoder, &info);
...
static void bt_conn_get_remote_info_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
	int result;
	struct bt_conn *conn;
...

	{
		...
		bt_conn_remote_info_enc(&ctx.encoder, &remote_info);
...
static void bt_conn_le_param_update_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
...
#if defined(CONFIG_BT_USER_DATA_LEN_UPDATE)
void bt_conn_le_data_len_param_dec(CborValue *value, struct bt_conn_le_data_len_param *data)
{
	data->tx_max_len = ser_decode_uint(value);
	data->tx_max_time = ser_decode_uint(value);
}

static void bt_conn_le_data_len_update_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	...
	conn = bt_rpc_decode_bt_conn(value);
	bt_conn_le_data_len_param_dec(value, &param);
...
#if defined(CONFIG_BT_USER_PHY_UPDATE)
void bt_conn_le_phy_param_dec(CborValue *value, struct bt_conn_le_phy_param *data)
{
	data->options = ser_decode_uint(value);
	data->pref_tx_phy = ser_decode_uint(value);
	data->pref_rx_phy = ser_decode_uint(value);
}

static void bt_conn_le_phy_update_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	...
	conn = bt_rpc_decode_bt_conn(value);
	bt_conn_le_phy_param_dec(value, &param);
...
static void bt_conn_disconnect_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	uint8_t reason;
	int result;
...
void bt_conn_le_create_param_dec(CborValue *value, struct bt_conn_le_create_param *data)
{
	data->options = ser_decode_uint(value);
...
static void bt_conn_le_create_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
	int result;
	bt_addr_le_t peer_data;
	...
	peer = ser_decode_buffer(value, &peer_data, sizeof(bt_addr_le_t));
	bt_conn_le_create_param_dec(value, &create_param);
	bt_le_conn_param_dec(value, &conn_param);
...
static void bt_conn_create_auto_stop_rpc_handler(CborValue *value, void *handler_data)
{
	int result;
...
static void bt_conn_set_security_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	bt_security_t sec;
	int result;
	...
	conn = bt_rpc_decode_bt_conn(value);
	sec = (bt_security_t)ser_decode_uint(value);
...
static void bt_conn_get_security_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
...
static void bt_conn_enc_key_size_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	uint8_t result;
					     ...
					     latency, uint16_t timeout)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	ser_encode_uint(&ctx.encoder, interval);
	ser_encode_uint(&ctx.encoder, latency);
	ser_encode_uint(&ctx.encoder, timeout);
					      ...
					      bt_addr_le_t *identity)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	ser_encode_buffer(&ctx.encoder, rpa, sizeof(bt_addr_le_t));
	ser_encode_buffer(&ctx.encoder, identity, sizeof(bt_addr_le_t));
					     ...
					     enum bt_security_err err)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	ser_encode_uint(&ctx.encoder, (uint32_t)level);
	ser_encode_uint(&ctx.encoder, (uint32_t)err);
...
static void bt_conn_cb_le_phy_updated_call(struct bt_conn *conn, struct bt_conn_le_phy_info *param)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	bt_conn_le_phy_info_enc(&ctx.encoder, param);
						...
						struct bt_conn_le_data_len_info *info)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	bt_conn_le_data_len_info_enc(&ctx.encoder, info);
...
static void bt_conn_cb_register_on_remote_rpc_handler(CborValue *value, void *handler_data)
{
	nrf_rpc_cbor_decoding_done(value);
...
#if defined(CONFIG_BT_SMP)
static void bt_set_bondable_rpc_handler(CborValue *value, void *handler_data)
{
	bool enable;

	enable = ser_decode_bool(value);
...
static void bt_set_oob_data_flag_rpc_handler(CborValue *value, void *handler_data)
{
	bool enable;
...
static void bt_le_oob_set_legacy_tk_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	const uint8_t *tk;
	int result;
...
static void bt_le_oob_set_sc_data_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	...
	if (ser_decode_is_null(value)) {
		...
		bt_le_oob_sc_data_dec(value, oobd_local);
	}
	if (ser_decode_is_null(value)) {
		oobd_remote = NULL;
		ser_decode_skip(value);
		...
		bt_le_oob_sc_data_dec(value, oobd_remote);
	}
...
static void bt_le_oob_get_sc_data_rpc_handler(CborValue *value, void *handler_data)
{
	struct nrf_rpc_cbor_ctx ctx;
	int result;
	struct bt_conn *conn;
...
static void bt_passkey_set_rpc_handler(CborValue *value, void *handler_data)
{
...
void bt_conn_pairing_feat_enc(CborEncoder *encoder, const struct bt_conn_pairing_feat *data)
{
...
static void bt_rpc_auth_cb_pairing_accept_rpc_rsp(CborValue *value, void *handler_data)
{
	struct bt_rpc_auth_cb_pairing_accept_rpc_res *res =
		(struct bt_rpc_auth_cb_pairing_accept_rpc_res *)handler_data;

	res->result = (enum bt_security_err)ser_decode_uint(value);
}
						   ...
						   const struct bt_conn_pairing_feat *const feat)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	bt_conn_pairing_feat_enc(&ctx.encoder, feat);
...
void bt_rpc_auth_cb_oob_data_request(struct bt_conn *conn, struct bt_conn_oob_info *info)
{
	...
	bt_rpc_encode_bt_conn(&ctx.encoder, conn);
	ser_encode_buffer(&ctx.encoder, info, sizeof(struct bt_conn_oob_info));
...
static void bt_conn_auth_cb_register_on_remote_rpc_handler(CborValue *value, void *handler_data)
{
	uint16_t flags;
	int result;
...
static void bt_conn_auth_cancel_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	int result;
...
static void bt_conn_auth_passkey_confirm_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	int result;
...
static void bt_conn_auth_pairing_confirm_rpc_handler(CborValue *value, void *handler_data)
{
	struct bt_conn *conn;
	int result;
